...
// constructor that take in multiple null terminated strings at once, the last
// argument has to be NULL
string_t* str_new_strings(const char* src, ...);

// constructor that take in an array of c strings, the last element of the array
// has to be NULL
string_t* str_new_string_arr(const char** src);

// exactly the same with with str_new_string_arr, but you need to input array
// size manually
...
// the array must be allocated on the heap since this function also calls free
// on the array. this function is intended to free the array returned by this
// library
void str_free_arr(string_t** str_arr);

// effectivly clear the string, set the length to 0, may or may not change the
// capacity.
...
// append two string together, return address would be the same as start. return
// NULL when failure
...
// add null terminated c type strings to string_t, the last element has to be
// NULL, return a new string_t*
string_t* str_appended_cstrs(const string_t* start, ...);

// add null terminated c type strings to start, the last element has to be NULL,
// return new start.
string_t* str_append_cstrs(string_t** start, ...);

// split src string upon needle string, returns an array of string_t with the
// last element being NULL caller free, use `str_destroy_string_arr` to free the
// returned array
...
// sort string array of size size, if size is 0, then the array has to be NULL
// terminated mode should be l for length, a for alphabetical, i for ignore
// case, c for custom using "a" or "ai" for multibytes string may not return the
// correct result if used c, other modes will be ignored order does matter for
// "l" and "a", "la" and "al" will return different result
...
// if new_val is 0, return the character at index index. Otherwise change that
// character to new_val
...
// convert string_t to long int. If err is `NULL`, there will not be error
// handling `err` is `true` if succeed, `false` if failed, base is used to tell
// the base of src, if base 0 or base 16, src can start with '0x' or '0X'
long str_strtol(const string_t* src, bool* err, int base);

// utf-8 version of char at, can not change character.
// this function returns a address of global variable buffer, user does not need
// to free the buffer but it is user's responsibility to `strdup` this buffer
// for latter use.
